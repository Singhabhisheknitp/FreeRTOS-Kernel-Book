...
- They can complicate the linker configuration. HOW EXACTLY??????????????????????
...
DOUBT: UNLIKE OS KERNELS WHICH HAS FIXED SIZE OF MEMORY SPACE, AND WHEN IT CRETAES EXTRA PROCESS ALLOCATE THEM DIFFERENT ADDRESS SPACE IN DRAM , WHICH IS NOT COUNTED AS OS KERNEL ADDRESS SPACE, HERE ALL THE ADDRESS CREATION THAT IS HAPPENING BY FREERTOS FOR APPLICATIONS, IT SUBDIVIDING ONE BIG AREA AND GIVING IT TO APPLICATION SO IN AWAY IT SHOULD BE PART OF APPLICATION BUT THE ADDRESS IS ASSCOTATED WITH FREERTOS HOW???? BECAUSE WE ARE SAYING "FreeRTOS appear to consume a lot of RAM because
the heap becomes part of the FreeRTOS data."
...
- **C** shows the situation after creating another task. Creating
  ...
  DOUBT: HERE IF NEW TASK SIZE WAS BIGGER THAN STACK USED BY PREVIUSLY USED TASK THAT GOT DELETED THEN IN THAT CASE THE UPPER STACK PORTION WILL BE USED FOR NEW TASK HENCE TCB & STACK OF TASK WILL BE AT DIFFERENT PLACES. IS THAT OKAY???????????????????
...
DOUBT: IS IT LIKE WHEN FREERTOS IS RUNNING AND HAVE ALLOCATED FEW HEAPS TO SOME PROCESS THEN THERE MAY SOME NON-CONTINEOUS PROCESS SO THIS SCHEME COMBINES THOSE DISCONTINEOUS SPACE TO CREATE A SINGLE HEAP FOR CERTAIM PROCESS????????
LINUX WHILE CREATING ADDRESS SPACE FOR PROCESSES DOES IT ALWAYS USE CONTINEOUS RAM SPACE OR COMBINE RAM SPACE FROM DIFFERENT DISCONTINOEUS CHUNK TO CREATE A ADDRESS SPACE PHYSICAL PORTION.
...
- The build tools will not know, and therefore cannot warn the
  ...
  DOUBT: LINKER IS ALSO AN APPLICATION LIKE OTHER APPLICATION THAT FREERTOS DECLARES SOME STACK SPACE IN RAM, SO WHY CAN NOT FREERTOS GIVES SOME CONSTANT SIZE TO THIS ALSO?? LIKE ANY OTHER APPLICATION??
